...
/// See also:
///
///   * [PreviewMask.builder], whose builder is of this type.
...
/// {@end-tool}
@immutable
class PreviewMask extends StatefulWidget {
  ...
  ///
  /// Passed with the [PreviewMask.builder] constructor. Otherwise, the
  ...
  ///
  /// If the [PreviewMask.builder] constructor is used, then this will be
  ...
  ///
  /// At the time this is called, the [PreviewMaskTransformationController] will have
  ...
  ///
  /// At the time this is called, the [PreviewMaskTransformationController] will not have
  ...
  /// [PreviewMaskTransformationController.toScene] for how to convert these coordinates to
  ...
  /// A [PreviewMaskTransformationController] for the transformation performed on the
  /// child.
  ///
  ...
  final PreviewMaskTransformationController? transformationController;
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      PreviewMask.getNearestPointOnLine(point, quad.point0, quad.point1),
      PreviewMask.getNearestPointOnLine(point, quad.point1, quad.point2),
      PreviewMask.getNearestPointOnLine(point, quad.point2, quad.point3),
      PreviewMask.getNearestPointOnLine(point, quad.point3, quad.point0),
    ];
  ...
  @override
  State<PreviewMask> createState() => PreviewMaskState();
}

class PreviewMaskState extends State<PreviewMask> with TickerProviderStateMixin {
  PreviewMaskTransformationController? _transformationController;
  ...
  void initState() {
    ...
    _transformationController = widget.transformationController
        ?? PreviewMaskTransformationController();
  ...
  @override
  void didUpdateWidget(PreviewMask oldWidget) {
...
/// See also:
///
///  * [PreviewMask.transformationController] for detailed documentation
///    on how to use TransformationController with [InteractiveViewer].
class PreviewMaskTransformationController extends ValueNotifier<Matrix4> {
  /// Create an instance of [PreviewMaskTransformationController].
  ///
  /// The [value] defaults to the identity matrix, which corresponds to no
  /// transformation.
  PreviewMaskTransformationController([Matrix4? value]) : super(value ?? Matrix4.identity());
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return PreviewMask.getAxisAlignedBoundingBox(boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside = PreviewMask.getNearestPointInside(point, boundary);
